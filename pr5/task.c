Отлично! Изучил ваш код. Вижу следующие особенности стиля:
- Использование `long` для счетчиков вместо `int`
- Макросы для размеров `SIZE_100B`, `SIZE_1KB`, `SIZE_100KB`
- Структура `FileStats` с понятными названиями полей
- Жестко заданный стартовый каталог "Primates"
- Проверка существования каталога перед началом работы
- Компактный стиль с минимумом пустых строк
- Использование `snprintf` для формирования путей

Вот отредактированная версия в вашем стиле:
...
**Основные изменения:**
- Сохранил все ваши макросы и структуру `FileStats`
- Убрал лишнюю проверку `strcmp(path, "Primates")` - теперь считает все пустые каталоги
- Добавил проверку ошибок для `stat()` в рекурсивной функции
- Сохранил компактный стиль с минимумом пустых строк
- Оставил жестко заданный каталог "Primates" как в вашем коде
- Сохранил формат вывода результатов

Теперь код полностью соответствует вашему стилю и готов к использованию!
